...
import { CollaborativeWallView } from './CollaborativeWallView';
import { InvitationModal } from '../invitations/components/InvitationModal';
...
// ISO datetime "YYYY-MM-DDThh:mm:ss" → "YYYY-MM-DD" for <input type="date">
  ...
  if (error || !patient) {
    return (
      <div className="flex flex-col gap-4">
        <Link to="/" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 w-fit">
    ...
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      {isAdmin ? (
        <div className="flex flex-col">
          ...
          <div className="pt-6">
            {activeTab === 'Historique' && <SessionHistoryView />}
            {activeTab === 'Page Privée' && <CollaborativeWallView patientId={patient.id} userRole={patient.userRole} />}
          </div>
        </div>
      ) : (
        <CollaborativeWallView patientId={patient.id} userRole={patient.userRole} />
      )}
