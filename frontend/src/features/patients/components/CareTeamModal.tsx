    ...
    >
      <div className="bg-white rounded-2xl border border-sand-200 shadow-xl w-full max-w-md p-6 flex flex-col gap-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-ink">Membres de l'équipe</h3>
          <button onClick={onClose} className="text-taupe-400 hover:text-ink" aria-label="Fermer">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex justify-center py-8">
            <Loader2 className="h-5 w-5 animate-spin text-taupe-400" />
          </div>
        ) : error ? (
          ...
          <p className="text-sm text-taupe-400 text-center py-8">Aucun membre dans l'équipe.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-full bg-petrol-50 flex items-center justify-center shrink-0">
                      <UserRound className="h-4 w-4 text-petrol-600" />
                    </div>
                    <div className="flex flex-col min-w-0">
                      <span className="text-sm font-medium text-ink truncate">
                        {member.lastName.toUpperCase()}, {member.firstName}
                      </span>
                      <span className="text-xs text-taupe-500 truncate">{member.relationship}</span>
                    </div>
                  </div>
