...
export function PostCard({ post, patientId, currentUserId, userRole, onUpdated, onDeleted }: PostCardProps) {
  ...
  async function handleSavePost() {
    ...
    setSaving(true);
    try {
      const updated = await updatePost(patientId, post.id, {
  ...
  async function handleAddComment(e: { preventDefault(): void }) {
    ...
    try {
      const newComment = await createComment(patientId, post.id, { content: commentInput.trim() });
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {allowed && !editing && (
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={async () => { await deletePost(patientId, post.id); onDeleted(post.id); }} className="text-slate-400 hover:text-red-500 transition-colors">
      ...
      {editing ? (
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button size="sm" loading={saving} onClick={handleSavePost}>Enregistrer</Button>
          </div>
        </div>
      ) : isPurged ? (
