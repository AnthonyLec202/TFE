import { db, SessionStatus } from '../../../core/offline/LocalDatabase';
...
export async function getAllSessions(): Promise<LocalSession[]> {
  ...
  // The general dashboard shows only ACTIVE work: exclude sessions with a final attendance
  // status (archived to the patient's history) and any awaiting server-side deletion so they
  // vanish immediately.
  return sessions.filter(s => s.status === SessionStatus.Scheduled && s.syncStatus !== 'pending_delete');
}
...
export async function getSessionsForPatient(patientId: string): Promise<LocalSession[]> {
  ...
  // A patient's history holds any session with a final attendance status; exclude tombstoned ones.
  return sessions.filter(s => s.status !== SessionStatus.Scheduled && s.syncStatus !== 'pending_delete');
}

export async function setSessionStatusLocally(id: string, status: SessionStatus): Promise<void> {
  const existing = await db.sessions.get(id);
  if (!existing || existing.status === status) return;

  // Preserve pending_create so a not-yet-synced session still flows through the create path;
  // otherwise mark it pending_update for the SyncEngine to push the new status via PUT.
  const syncStatus: SyncStatus = existing.syncStatus === 'pending_create' ? 'pending_create' : 'pending_update';
